...
const AuthDataForm = ({ prevStep }) => {
  ...
  const handleChange = (e) => {
    ...
    if (e.target.name === "verifyPassword") {
      setPasswordMatchError(
        e.target.value !== localData.password
    ...
    if (e.target.name === "verifyEmail") {
      setEmailMatchError(
        e.target.value !== localData.email
  ...
  const handleSubmit = async () => {
    if (
      localData.password !== localData.verifyPassword ||
      localData.email !== localData.verifyEmail
    ) {
      return;
    }
    ...
    setIsLoading(true);
    const result = validateForm({ ...completeData });
    if (!result) {
      setIsLoading(false);
      return;
    }
